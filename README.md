...
| Команда | Назначение |
|---------|------------|
| `Toggle Number Highlighting` | Включить/выключить подсветку отдельно стоящих чисел |
...
2. Поиск целых отдельно стоящих чисел с помощью регулярного выражения `/\b\d+\b/g`
3. Классификация отдельно стоящих чисел на четные/нечетные
...
### `clearHighlights()`
**Назначение**: Удаление всех выделений отдельно стоящих чисел из активного редактора

**Действие**: Очищает декорации для четных и нечетных отдельно стоящих чисел
...
### `toggleHighlighting()`
**Назначение**: Переключение состояния подсветки чисел отдельно стоящих
